...
.DESCRIPTION
The Get-PrtgVersion cmdlet retrieves version details found in various locations in the PrtgAPI project.
Version details can be updated using the Set-PrtgVersion and Update-PrtgVersion cmdlet. The following
table details the version details that can be retrieved:
...
Note that if PrtgAPI detects that the .git folder is missing from the repo or that the "git" command is
not installed on your system, the PreviousTag property will be emitted from results.

.PARAMETER IsCore
Specifies whether to retrieve the versions used when compiling PrtgAPI using
...
function Get-PrtgVersion
{
    ...
    Get-CIVersion $root -IsCore:$IsCore -ErrorAction $ErrorActionPreference
}