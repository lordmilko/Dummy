...
.DESCRIPTION
The Set-PrtgVersion cmdlet updates the version of PrtgAPI. The Set-PrtgVersion cmdlet allows
the major, minor, build and revision components to be replaced with any arbitrary version. Typically
the Set-PrtgVersion cmdlet is used to revert mistakes made when utilizing the Update-PrtgVersion cmdlet
...
Set the version to version 1.2.3.4. Systems that only utilize the first three version
components will be versioned as 1.2.3
...
function Set-PrtgVersion
{
    ...
    $old = Get-PrtgVersion -IsCore:$IsCore -ErrorAction SilentlyContinue

    SetPrtgVersionInternal $Version $IsCore

    $new = Get-PrtgVersion -IsCore:$IsCore -ErrorAction SilentlyContinue
