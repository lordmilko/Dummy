...
console. By default, Start-PrtgAPI will attempt to launch the last Debug build of PrtgAPI.
Builds for .NET Core and .NET Standard will be launched in PowerShell Core, while builds for
the .NET Framework will be launched in Windows PowerShell. If builds for multiple target frameworks
are detected, Start-PrtgAPI will throw an execption specifying the builds that were found. A specific
target framework can be specified to the -TargetFramework parameter.

If -IsCore is false, Start-PrtgAPI will skip enumerating target frameworks and instead attempt to open
a build from the legacy .NET Framework version of PrtgAPI in a Windows PowerShell console.
...
function Start-PrtgAPI
{
    ...
    $path = GetModulePath $Configuration $IsCore $TargetFramework

    if(Test-Path $path)
    {
        $exe = GetPowerShellExe $path $IsCore

        $prtgAPIDir = Join-Path $path "PrtgAPI"
        $psd1 = Join-Path $prtgAPIDir "PrtgAPI.psd1"

        if(Test-IsWindows)
        {
            Write-Host -ForegroundColor Green "`nLaunching PrtgAPI from '$prtgAPIDir'`n"
        ...
        else
        {
            ...
            Write-Host -ForegroundColor Green "`nImporting PrtgAPI from $prtgAPIDir"
...
function GetPowerShellExe($path, $isCore)
{
    if($isCore)
    {
        if($path -notlike "*net4*")
        {
            ...
            {
                throw "Cannot find command 'pwsh' for launchign module '$path'; is PowerShell Core installed?"
            }
...
function GetModulePath($configuration, $IsCore, $targetFramework)
{
    ...
    $bin = Join-Path (Join-Path $root "PrtgAPI.PowerShell") "bin"

    if($IsCore -and ![string]::IsNullOrWhiteSpace($targetFramework))
    {
        $targetFolder = "$bin\$configuration\$targetFramework"

        if(!(Test-Path $targetFolder))
        {
            if(Test-Path "$bin\$configuration")
            {
                $candidates = gci "$bin\$configuration" -Filter "net*"
...
function CalculateTargetFolder
{
    if($IsCore)
    {
        $configFolder = "$bin\$configuration"
