...
.DESCRIPTION
The Invoke-PrtgAnalyzer cmdlet analyzes best practice rules on the PrtgAPI repository on all
PowerShell script files. By default, Invoke-PrtgAnalyzer will report violations on all files
in the PrtgAPI repository. This can be limited to a particular subset of files by specifying
a wildcard to the -Name parameter.

For certain rule violations, Invoke-PrtgAnalyzer can automatically apply the recommended fixes
for you by specifying the -Fix parameter. To view the changes that will be applied it is
recommended to also apply the -WhatIf parameter, and to have a clean Git working directory
so that you may undo all of the changes or apply them in a single commit as desired.
