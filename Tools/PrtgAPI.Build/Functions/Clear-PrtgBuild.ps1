...
.DESCRIPTION
The Clear-PrtgBuild clears the output of previous builds of PrtgAPI. By default,
Clear-PrtgBuild will attempt to use the appropriate build tool (msbuild or dotnet.exe)
to clear the previous build. If If -Full is specified, Clear-PrtgBuild will will instead
force remove the bin and obj folders of each project in the solution.

.PARAMETER Configuration
Configuration to clean. If no value is specified PrtgAPI will clean the last Debug build.

.PARAMETER IsCore
Specifies whether to use the .NET Core CLI to clear the build.
