...
.DESCRIPTION
The Get-PrtgCoverage cmdlet generates a code coverage report for PrtgAPI. By default, all tests
will be executed using the Debug configuration. Coverage analysis can be limited to a subset
of tests by specifying a wildcard to the -Name parameter.

When the coverage analysis has completed, a HTML report detailing the results of the analysis
will automatically be opened in your default web browser.
...
.PARAMETER IsCore
Specifies whether code coverage should be generated using .NET Core CLI tooling (where applicable)
